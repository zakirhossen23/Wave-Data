...
class AuthScreenApp extends State<AuthScreen> {
  ...
  Future<void> LoginAccount() async {
    var url = Uri.parse(
        'https://cors-anyhere.herokuapp.com/https://wavedata.i.tgcloud.io:14240/restpp/query/WaveData/Login?emailTXT=${Uri.encodeComponent(emailTXT.text)}&passwordTXT=${Uri.encodeComponent(passwordTXT.text)}');
    final response = await http.get(url, headers: TGheader);
    var responseData = json.decode(response.body);
    var data = (responseData['results']);
    if (data[1]['(SV.size())'] == 1) {
      var userid = data[0]['(SV)'][0]['v_id'];
      print(userid);
      // Obtain shared preferences.
      final prefs = await SharedPreferences.getInstance();
